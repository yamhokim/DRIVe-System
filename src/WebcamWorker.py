...
import imutils
from csv_writer import write_to_csv, write_to_csv_perclos
...
class WebcamWorker(QThread):
    ...
    def __init__(self, time_secs, ear_vals, mar_vals, ear_and_mar_vals, blink_list, yawn_list, temp_ear_data, temp_time_data, ear_threshold, ear_open_value, perclos_vals, dimensions):
        ...
        self.header_counter = 0 # This is a counter for the log_data function to ensure the headers are only written once
                        ...
                        if results.multi_face_landmarks == None:
                            ...
                            for face_landmarks in results.multi_face_landmarks:
                                ...
                                if time_elapsed_temp >= 10:  # 1 minute has passed, we are finished the interval
                                    ...
                                    self.perclos_vals.append((self.interval_counter, perclos))
                                    closed_frame_counter = 0
                                    total_frame_counter = 0
    ...
    def start_rating_countdown(self):
        ...
        self.log_and_clear_data()
    

    def create_participant_info(self, participant_info):
        self.participant_info = participant_info


    def log_and_clear_data(self):
        write_to_csv(self.participant_info, self.ear_and_mar_vals, self.header_counter)
        write_to_csv_perclos(self.participant_info, self.perclos_vals)

        # If you don't really care about the plots, erase these two lines or comment them out
        #plot_eye_data(self.time_secs, self.ear_vals, self.blink_list, self.participant_info, self.interval_counter)
        #plot_mouth_data(self.time_secs, self.mar_vals, self.yawn_list, self.participant_info, self.interval_counter)

        # Clear self.ear_vals, self.mar_vals, self.ear_and_mar_vals, self.time_secs, self.blink_list, self.yawn_list, self.temp_ear_data, self.temp_time_data to free up memory
        self.clear_data()
        self.header_counter = 1 # Set this to one since the headers will now have been written


    def clear_data(self):
        self.ear_vals = []
        self.mar_vals = []
        self.ear_and_mar_vals = []
        self.time_secs = []
        self.blink_list = []
        self.yawn_list = []
        self.perclos_vals = []
        
        self.temp_ear_data = self.temp_ear_data[-60:]
        self.temp_time_data = self.temp_time_data[-60:]
